...
describe('parseAgency', () => {
  it('parses a minimal valid agency.txt', () => {
    const csv = [
      'agency_name,agency_url,agency_timezone',
      'CTP Cluj-Napoca,https://www.ctpcluj.ro/,Europe/Bucharest',
    ].join('\n');
    const rows = parseAgency(csv);
  ...
  it('strips the UTF-8 BOM', () => {
    ...
    expect(parseAgency(csv)).toHaveLength(1);
  });

  it('rejects a row with empty agency_name', () => {
    const csv = 'agency_name,agency_url,agency_timezone\n,https://ctp.ro/,Europe/Bucharest';
    expect(() => parseAgency(csv)).toThrow();
  });

  it('returns an empty array for header-only input', () => {
    const csv = 'agency_name,agency_url,agency_timezone';
    expect(parseAgency(csv)).toEqual([]);
  });
});
